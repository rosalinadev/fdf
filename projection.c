...
/*   Updated: 2024/01/28 03:10:27 by rvandepu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_point	proj2d(t_fdf *fdf, t_coords c, t_btree *node)
{
	...
	p.pos.x = (c.x - ((fdf->mesh->width - 1) / 2.0)) * 10
		+ (fdf->width / 2.0);
	p.pos.y = (c.y - ((fdf->mesh->height - 1) / 2.0)) * 10
		+ (fdf->height / 2.0);
...
t_point	proj3d(t_fdf *fdf, t_coords coords, t_btree *node)
{
	...
	t = fdf->rot;
	a.x = coords.x - ((fdf->mesh->width - 1) / 2.0);
	a.z = coords.y - ((fdf->mesh->height - 1) / 2.0);
	a.y = -node->value * fdf->scale;
